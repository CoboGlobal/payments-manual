...
First, choose a server environment, such as a cloud service like AWS, Google Cloud, or a self-hosted server, that supports receiving and processing webhook events. Then, define an endpoint URL on your server where the webhook events will be sent. The endpoint URL must use HTTPS to protect webhook payloads in transit.
...
The webhook request body is a JSON object. Read the `type` field to determine the event type and route your handling logic accordingly. The `data` field contains the event-specific payload.
...
> **Note:** The top-level field name is `type`, not `event_type`.
...
1. Retrieve raw body and timestamp. 
    ...
    Extract the original body string from the request payload and the timestamp (`biz_timestamp`) from the request headers.

    ```python
    raw_body = request.body().decode('utf8')
    biz_timestamp = request.headers.get("BIZ_TIMESTAMP")
    ```
...
3. Concatenate and hash the message.
    ...
    message = f"{raw_body}|{biz_timestamp}"
...
5. Verify the signature using the Ed25519 algorithm.
    ...
    <Note>Install the required library before running this code: `pip install pynacl`</Note>

    ```python
    # Install required library: pip install pynacl
...
If signature verification fails, return HTTP 401. Returning `200` or `201` on a failed verification will cause the event to be incorrectly marked as Delivered on Cobo Portal.
...
Cobo does not guarantee that events will be delivered in the order they are generated. For example, a deposit transaction in Top-up Mode may generate the following events:
...
<Tip>To test your endpoint without creating real transactions, use the Webhook Trigger API (`POST /payments/webhooks/trigger`), available since SDK v1.32.0 (March 2026). This API lets you manually trigger any Payments webhook event type against your registered endpoint.</Tip>
