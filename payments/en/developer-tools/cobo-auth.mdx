...
<Info>For more details about permissions and wallet scopes, see [Permissions and wallet scopes](/payments/en/developer-tools/permissions-and-scopes).</Info>
...
1. Install Cobo CLI using the following command:
    ...
    For more details about the system requirements for Cobo CLI, please refer to [Install Cobo CLI](/payments/en/developer-tools/cobo-cli/quick-start-guide#system-requirements).
...
2. Use the `keys generate` command to generate a key pair using the Ed25519 algorithm as follows:
    ...
    Generated keys are securely stored in your configuration file (default: `~/.cobo/config.toml`). For more information about this command, refer to [Key management](/payments/en/developer-tools/cobo-cli/key-management).
