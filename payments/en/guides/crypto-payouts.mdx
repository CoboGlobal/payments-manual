...
You can subscribe to the `payment.payout.status.updated` event to receive real-time updates on transfer status. Refer to [Webhook reference](/payments/en/guides/events/payouts) to understand the detailed trigger conditions and returned data structure of each event.
