...
<Note>
  This automatic collection happens as an internal step of the payout process. It is separate from the (deprecated) `Create forced sweep` and `List forced sweeps` operations, which previously let you trigger fund collection manually and are no longer necessary.
</Note>
...
Payout reversal is when funds from a crypto payout you already completed are sent back on-chain to a Payment collection or source address. This situation is not initiated or resolved with the [Create Refund API](/payments/en/api-references/payment/create-refund-order), because Create Refund is for pay-in order refunds.

Returned funds are credited automatically. After the returned funds are detected and pass compliance screening, they are handled as an unexpected incoming transaction and credited to your developer balance. Cobo does not need to intervene manually.

Cobo sends the `payment.transaction.external.completed` event when the returned transaction completes, so you can reconcile the credited amount. The original payout remains a completed payout record; returned funds do not create a dedicated payout reversal status on that payout.
