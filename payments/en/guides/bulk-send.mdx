...
For Bulk Send on Ethereum, Cobo uses smart contracts to perform batch transfers. For other chains, Cobo uses regular transactions instead. In both cases, Cobo sends a callback message to your registered callback endpoint for each operation so that you can inspect the details and approve or deny execution. The exact mechanism may also vary based on your organization's configuration. To learn how to set up and register a callback endpoint, see [Introduction to Callbacks](/payments/en/developer-tools/callback).
