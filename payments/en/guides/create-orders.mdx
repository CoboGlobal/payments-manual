...
For Payin order status changes and transaction events, use [Status and events](/payments/en/guides/status-and-events). For frozen-funds disposition state, call `get_disposition_status` and inspect `disposition_status`.
...
| Operation | Purpose | What you can observe |
| --- | --- | --- |
| `unfreeze_funds` | Releases frozen funds after compliance review. | Call `get_disposition_status` and inspect `disposition_status` to track the resulting state. |
| `isolate_funds` | Isolates frozen funds after compliance review. | Call `get_disposition_status` and inspect `disposition_status`; relevant values are `Isolating`, `Isolated`, and `IsolateFailed`. |
| `refund_funds` | Returns frozen Payin funds through a compliance-managed return flow. | Call `get_disposition_status` and inspect `disposition_status` to track the resulting state. This is distinct from the developer-initiated [Create Refund API and refund link flows](/payments/en/guides/create-refund-link#refund-scenarios). |
...
| Release frozen funds with `unfreeze_funds`. | This is a compliance-managed disposition operation. Call `get_disposition_status` and inspect `disposition_status` to observe the resulting state. |
| Isolate frozen funds with `isolate_funds`. | This is a compliance-managed disposition operation. Call `get_disposition_status` and inspect `disposition_status` to observe the resulting state. |
| Return frozen Payin funds with `refund_funds`. | This is a compliance-managed disposition operation, distinct from the developer-initiated [Create refund order](/payments/en/api-references/payment/create-refund-order). Call `get_disposition_status` and inspect `disposition_status` to observe the resulting state. |
...
- To optimize your account costs and prevent situations where the collection fee exceeds the transaction value, a minimum deposit threshold is applied. Transactions with a value below **0.05 USDT** (or equivalent) will not be processed for automatic credit.\
  _Note: If you need to manually recover accumulated funds below this threshold, submit the request through your account process._
