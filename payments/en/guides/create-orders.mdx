...
<Info>
  **Refund flows**: `POST /payments/refunds` (the Create Refund API) is the developer-initiated direct API flow. You can supply `to_address` in the create request; if you omit it, the refund is created in `PendingConfirmation` status and you must provide the address later using the Update refund order operation before processing can proceed. The API does not auto-fill the original pay-in address. This is separate from the [refund link](/payments/en/guides/create-refund-link) flow, where Cobo provides an address-collection page that pre-fills the original payment address for the payer. Two further scenarios are not initiated through `POST /payments/refunds`, and they differ from each other: **payout reversal** is when funds from a payout you already completed are sent back to a Payment address, so funds flow in — see [Crypto payouts](/payments/en/guides/crypto-payouts#5-payout-reversal); **returning compliance-frozen funds** is when a frozen incoming payment is sent out again through the compliance funds disposition flow, so funds flow out, using `refund_funds`.
</Info>
...
| `refund_funds` | Returns frozen Payin funds through a compliance-managed return flow. | Compliance-managed. Distinct from the developer-initiated [Create Refund API and refund link flows](/payments/en/guides/create-refund-link#types-of-fund-returns). |
...
To optimize your account costs and prevent situations where the collection fee exceeds the transaction value, a dust amount threshold is applied. Transactions with a value **equal to or below 0.05 USDT** (or equivalent) are not processed for automatic credit, do not count toward the order's received amount, and do not trigger a webhook notification.
