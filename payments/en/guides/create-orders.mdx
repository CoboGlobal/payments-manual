...
**Only a transaction strictly greater than the threshold is credited.** A transaction equal to or below the threshold is treated as dust: it is not credited, it does not count toward the order's received amount, and no webhook notification is sent.
