...
<Tabs>
  ...
  <Tab title="Create order link" icon="browser">
    ...
    5. Cobo sends a [`payment.order.status.updated`](/payments/en/guides/events/orders) webhook event with status `Pending` and the new `order_id` to the merchant.
...
You can subscribe to the following webhook events to receive real-time update notifications of order status. Refer to [Webhook reference](/payments/en/guides/events/orders) to understand the trigger time and returned data structure of each event.
...
You can subscribe to the `payment.refund.status.updated` event to receive real-time updates on refund order status. Refer to [Webhook reference](/payments/en/guides/events/refunds) to understand the detailed trigger conditions and returned data structure of each event.
