...
When a Settlement Network transaction completes, the settled funds reach your Payment account balance and Cobo emits [`payment.account.balance.updated`](/payments/en/guides/events/balances) with the `SettlementNetwork` `source_type`. That event carries the amount that landed.
