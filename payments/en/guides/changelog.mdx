...
| Feature | Preview |
|---------|---------|
| **Funds Management** | - New bulk payout feature for transferring cryptocurrency to multiple addresses at once.<br/>- New Allocation API operations for flexible fund distribution between accounts.<br/>- New Payout API operations with streamlined process—no need to specify fund source (pay-in/top-up mode) or orders, just enter the amount. |
| **Pay-in** | - Simplified fields for [Create pay-in order](/payments/en/api-references/payment/create-pay-in-order) and [Create order link](/payments/en/api-references/payment/create-order-link): support for custom payable amounts, removal of exchange_rate field, and optimized field naming for better readability.<br/>- Global precision for amount fields unified to 2 decimal places for easier reconciliation and fund management. |
| **Payment Link** | When creating an order payment link via [Create order link](/payments/en/api-references/payment/create-order-link), you can now configure the merchant's original shopping page URL. Users can be redirected back to the shopping page upon successful payment or errors, improving the payment experience. |
