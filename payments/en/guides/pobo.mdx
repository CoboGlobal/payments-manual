...
Payouts on behalf of (POBO) lets you pay your counterparties in fiat without holding bank accounts or local payment rails yourself. You instruct the payout from your Cobo balance, and the funds are converted and transferred to the recipient's bank account. Both sides are verified first: your business through [KYC](/payments/en/guides/glossary#kyc) (verifying the entity's identity and who controls it), and each recipient bank account through destination review.

Typical uses are paying suppliers, contractors, or partners in markets where you have no banking presence.

This guide shows the POBO implementation sequence with the Cobo WaaS 2.0 Python SDK (`cobo_waas2`): SDK initialization, KYC and bank account document upload, KYC submission, bank-account destination creation, and Off-ramp payout creation. For concept details and operation-specific schemas, use the related guides and API references linked from each section.
...
- You have a Payments API host, such as `https://api.sandbox.cobo.com/v2` for Sandbox.
- You have prepared the KYC documents and bank account documents required for the entity and payout destination.
...
Call `submit_merchant_kyc` to submit KYC information. The example submits B2B entity information, company attachments, legal representative information, and ultimate beneficial owner information.
...
Call `create_destination` to create the bank-account destination used by an Off-ramp payout. The example uses a company bank account with `BankAccountPaymentMethod.SWIFT` and `BankAccountHolderType.COMPANY`.
...
Call `create_payout` to submit the payout request. Generate a unique `request_id` for idempotency, set `payout_channel` to `PayoutChannel.OFFRAMP`, provide the source account, and reference the destination bank account in `recipient_info.bank_account_id`.
