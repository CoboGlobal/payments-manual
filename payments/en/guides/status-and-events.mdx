...
The shape of these objects is indicated by their nested `*_type` field (e.g. `source_type`, `destination_type`).

- `source` (object, required) — Where the deposit came from, typically an external wallet address (`DepositFromAddress`).
- `destination` (object, required) — Where the deposit went, typically a deposit address (`DepositToAddress`).
