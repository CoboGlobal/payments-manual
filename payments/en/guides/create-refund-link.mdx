...
| Scenario | How it is initiated | What it is for |
| --- | --- | --- |
| Create Refund Order | You create an ordinary refund order through Payments App or the [Create refund order](/payments/en/api-references/payment/create-refund-order) API. | Developer-initiated refunds after you decide to return funds to a payer. |
| Refund Link | You call [Create refund link](/payments/en/api-references/payment/create-refund-link) so the payer can submit a refund receiving address. | Ordinary refund address collection when you need the payer to provide or confirm the refund address. |
| Compliance return | `refund_funds` is a compliance-managed funds disposition operation, distinct from the Create Refund Order and Refund Link flows. | Returning frozen Payin funds after compliance review. |
| Payout Reversal | A separate reversal scenario. | Do not treat it as a Create Refund Order, Refund Link, or `refund_funds` flow. |

<Note>
  A compliance return through `refund_funds` is not the same as a Create Refund Order or Refund Link.
</Note>
...
Use Create Refund API or Create Refund Link only for pay-in order refunds. Other returned-fund situations have different triggers, entry points, fund directions, and status surfaces.

| Type | Trigger | Initiator | Fund direction | Entry point and status |
| :--- | :------ | :-------- | :------------- | :--------------------- |
| Payout reversal | Funds from a crypto payout you already completed are sent back on-chain to a Payment collection or source address. | Initiated on-chain by the other party; no action from you. Returned funds are credited automatically and no manual intervention is required. | External recipient address to a Payment address. | Not Create Refund API or Create Refund Link. See [Payout reversal](/payments/en/guides/crypto-payouts#5-payout-reversal); returned funds are handled as unexpected incoming funds and do not create a payout reversal status on the original payout. |
| Compliance/frozen-fund return | Compliance handling requires inbound funds to be returned through the compliance fund disposition process. | Compliance workflow. | Frozen inbound funds to the disposition destination. | Not Create Refund API or Create Refund Link; use the compliance fund disposition process. |
| Pay-in order refund | A payer or merchant needs to refund funds from a completed pay-in order. | Merchant, developer, or user refund flow. | Merchant or developer balance to the refund receiving address. | Use Create Refund API or Create Refund Link and track `payment.refund.status.updated`. |
...
<Note>
  The original payment address pre-fill shown on this page is specific to the refund-link address-collection flow. When you create a refund directly using the Create Refund API (`POST /payments/refunds`), the API does not auto-fill the original payment address. You can supply `to_address` in the create request; if you omit it, the refund is created in `PendingConfirmation` status and you must provide the address later using the Update refund order operation before processing can proceed. For Create Refund API behavior, see [Handle refund requests](/payments/en/guides/create-orders).
</Note>
...
If an ordinary refund address collection link has been used, call the same API to generate a new URL and send it to the user again. This applies only to the refund-link address-collection flow.
...
<Note>
  Regenerating a refund link is not a retry for a `refund_funds` failure. For ordinary refund orders, you can observe the `Failed` status and `failed_reason` in refund details.
</Note>
...
<Note>
  For other ordinary refund issues, query the refund order status and inspect `failed_reason` in the refund details.
</Note>
