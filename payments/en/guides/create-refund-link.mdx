...
| :--- | :------ | :-------- | :------------- | :--------------------- |
| Payout reversal | A recipient sends completed crypto payout funds back on-chain to a Payment collection or source address. | Recipient action. Returned funds are credited automatically; no manual intervention is required. | External recipient address to a Payment address. | Not Create Refund API or Create Refund Link. See [Payout reversal](/payments/en/guides/crypto-payouts#5-payout-reversal); returned funds are handled as unexpected incoming funds and do not create a payout reversal status on the original payout. |
