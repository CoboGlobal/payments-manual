...
| :--- | :------ | :-------- | :------------- | :--------------------- |
| Payout reversal | Funds from a crypto payout you already completed are sent back on-chain to a Payment collection or source address. | Initiated on-chain by the other party; no action from you. Returned funds are credited automatically to your developer balance and no manual intervention is required. | Funds in: from the external recipient address back to a Payment address. | Not Create Refund API or Create Refund Link. See [Payout reversal](/payments/en/guides/crypto-payouts#5-payout-reversal); returned funds are handled as unexpected incoming funds and do not create a payout reversal status on the original payout. |
| Compliance-frozen fund return | An incoming payment is frozen by compliance and must be returned through the compliance funds disposition flow. | Compliance workflow; you cannot initiate it yourself. | Frozen inbound funds to the disposition destination. | Not Create Refund API or Create Refund Link; use the compliance fund disposition process. |
