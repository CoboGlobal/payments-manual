...
To compare Top-up mode with Order mode and choose the mode that fits your business, refer to [Order mode vs. Top-up mode](/payments/en/guides/overview#order-mode-vs-top-up-mode).
...
After a Top-up deposit completes, `developer_fee_rate` splits the deposit into a merchant share and a developer share; each positive share is credited independently to its own account ledger, though a deposit whose developer share rounds down to zero posts only the merchant-side ledger entry. For a full worked example of this split, refer to [Accounts and fund allocation](/payments/en/guides/amounts-and-balances). To configure or adjust the rate, refer to [Merchants](/payments/en/guides/merchants). You can observe the gross amount received by calling [List payer transactions](/payments/en/api-references/payment/list-payer-transactions), the resulting account balances by calling [List merchant balances](/payments/en/api-references/payment/list-merchant-balances) and [Get developer balance](/payments/en/api-references/payment/get-developer-balance), and the individual ledger entries by calling [List balance changes](/payments/en/api-references/payment/list-balance-changes).
...
<Tabs>
  ...
  <Tab title="Payments API" icon="code">
    1. Call [Create/Get top-up address](/payments/en/api-references/payment/createget-top-up-address) to create a top-up address. Key parameters in the request include:
       ...
       If a top-up address already exists for the specified payer and chain, this operation returns that existing address instead of creating a new one, so you can call it repeatedly for the same payer and chain without creating duplicates.
    ...
    2. **Configure top-up event listening**: Receive event notifications related to top-ups through webhooks. For details, refer to [Events and status](/payments/en/guides/events/top-up).
  </Tab>
</Tabs>

## Get top-up mode related events

You can subscribe to the following webhook events to receive real-time update notifications for top-up addresses and top-up transactions. Refer to [Webhook reference](/payments/en/guides/events/top-up) to understand the trigger time and returned data structure of each event.
...
- **Fund handling**: The association between this address and the merchant still exists, so deposits continue routing to the same merchant after replacement. For the complete deposit-attribution and settlement model, see [Accounts and fund allocation](/payments/en/guides/amounts-and-balances#deposit-attribution). Before a payout, Cobo also automatically collects top-up funds from these addresses. For details, refer to [Automatic fund collection](/payments/en/guides/crypto-payouts#2-automatic-fund-collection).
...
To optimize your account costs and prevent situations where the collection fee exceeds the transaction value, a dust amount threshold is applied. Transactions with a value **equal to or below 0.05 USDT** (or equivalent) are not processed for automatic credit, and do not trigger a webhook notification.

For example:

| Transaction amount | Credited | Webhook sent |
|---|---|---|
| 0.06 USDT | Yes | Yes |
| **0.05 USDT** | **No** | **No** |
| 0.04 USDT | No | No |
