...
If you need to use a Merchant as the paying enterprise in an OffRamp payout to another enterprise's approved bank account, see [Enterprise third-party payouts](/payments/en/guides/enterprise-third-party-payouts).

When a payer completes a Top-up deposit, the system automatically allocates funds to the corresponding merchant account and developer account based on the merchant's configured developer fee rate. The following diagram shows the process of automatic fund allocation to corresponding accounts in Top-up mode. Order mode uses a separate, order-level charge instead — see [Payment collection in order mode](/payments/en/guides/create-orders) and [Accounts and fund allocation](/payments/en/guides/amounts-and-balances).
...
<Note>Completing [Payments onboarding](/payments/en/guides/preparation) automatically provisions a **default merchant** for your organization. If you serve payers directly and have no downstream merchants, you can use the default merchant directly without creating a new one.</Note>
...
1. Call [Create merchant](/payments/en/api-references/payment/create-merchant) to create a merchant, passing the following information in the request:  
   ...
   * `developer_fee_rate`: (Optional) Developer fee rate, applicable to Top-up mode only. Accepts a decimal fraction from 0 through 1, with up to four decimal places (for example, 0.015 for 1.5%). If not set, or if cleared, the rate is treated as 0. Updates take effect only for Top-up deposits processed after the change. For example, a 100 USDT Top-up deposit at a rate of 0.015 credits 1.5 USDT to the developer balance and 98.5 USDT to the merchant balance; the developer's share is rounded down to the token's supported precision.
     <Note>Order mode uses a separate, order-level developer charge (`fee_amount`) instead of `developer_fee_rate`. Because deposits are handled by separate Top-up and Order transaction logic, the two never apply to the same deposit, and there is no calculation order between them. For more details, refer to [Payment collection in order mode](/payments/en/guides/create-orders) and [Accounts and fund allocation](/payments/en/guides/amounts-and-balances).</Note>
