...
|---|---|---|---|
| 2006 | 400 | One or more parameters are in an invalid format or contain unsupported values. | Validate your request body against the [API reference](/payments/en/api-references/playground). Check parameter types, enum values, and all required fields. The `error_message` field usually names the specific parameter that failed validation. |
...
| 2027 | 400 | You have exceeded your quota limit for this resource or operation. | Check your current usage and quota limits in Cobo Portal under **Settings** > **Developer**. |
...
| 30002 | 400 | Unknown token ID. The specified token is not recognized. | Verify the token ID against the list of supported tokens. Call [List supported tokens](/payments/en/api-references/payment/list-supported-tokens) to retrieve valid token IDs for your organization. |
