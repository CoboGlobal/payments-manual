...
要了解事件类型和事件数据类型的信息，请参阅[事件与状态](/payments/cn/guides/events/overview)。
...
| `type` | string | 事件类型。所有事件类型及触发条件请参阅[事件与状态](/payments/cn/guides/events/overview)。 |
| `data` | object | 事件专属负载。结构取决于 `type` 字段——各事件的字段结构请参阅[事件与状态](/payments/cn/guides/events/overview)。 |
