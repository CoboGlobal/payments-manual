...
在 Ethereum 上进行批量下发时，Cobo 使用智能合约执行批量转账；其他链则使用普通交易。无论哪种方式，Cobo 都会对每笔操作向您注册的 Callback Endpoint 发送 Callback 消息，以便您检查详情并批准或拒绝执行。具体机制也可能因您的团队配置而有所不同。要了解如何设置和注册 Callback Endpoint，请参阅 [Callback 介绍](/payments/cn/developer-tools/callback)。
