...
POBO（Payouts on Behalf Of，全球付款）用于向与您不同主体的收款方支付法币，例如您的供应商、承包商或合作伙伴。您无需在当地自行持有银行账户：商户完成 [KYC](/payments/cn/guides/glossary#kyc) 审核后，Cobo 为其开立同名虚拟账户（VA），付款从该 VA 账户发出，收款方看到的汇款人即该商户主体本身。您从 Cobo 余额发起付款，资金经兑换后由该 VA 账户汇入收款方银行账户。

发起 POBO 付款前需完成两项审核，两者互相独立：

- **商户 KYC**：核验付款商户的主体身份与实际控制人，审核通过后为该商户开立同名 VA 账户。每个付款商户需单独提交。
- **转出目的地审核**：提交收款银行账户的转账信息，每个收款银行账户单独审核。

两项均可通过 API 提交；通过 Payments App 网页端提交预计于 2026 年 10 月支持。

本指南介绍 POBO 流程在 Cobo WaaS 2.0 Python SDK（`cobo_waas2`）上的实现顺序：SDK 初始化、商户 KYC 文件上传与信息提交（付款方）、收款银行账号文件上传与转出目的地创建（收款方），以及 Off-ramp 出金创建。
...
- 您已准备好商户主体和转出目的地所需的 KYC 文件及银行账户文件。
