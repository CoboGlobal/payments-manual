...
POBO（Payouts on Behalf Of，全球付款）让您无需自行持有银行账户或本地支付通道，即可向交易对手支付法币。您从 Cobo 余额发起付款，资金经兑换后汇入收款方银行账户。双方都需先通过审核：您的主体通过 [KYC](/payments/cn/guides/glossary#kyc) 审核（核验主体身份与实际控制人），每个收款银行账户通过转出目的地审核。

典型用途是向您没有银行业务布局的市场中的供应商、承包商或合作伙伴付款。

本指南介绍 POBO 流程在 Cobo WaaS 2.0 Python SDK（`cobo_waas2`）上的实现顺序：SDK 初始化、KYC 与银行账户文件上传、KYC 信息提交、银行账户转出目的地创建，以及 Off-ramp 出金创建。
...
- 您已获得 Payments API Host，例如测试环境 `https://api.sandbox.cobo.com/v2`。
- 您已准备主体和转出目的地所需的 KYC 文件及银行账户文件。
...
调用 `submit_merchant_kyc` 提交 KYC 信息。以下示例提交 B2B 主体信息、公司附件、法人信息和最终受益人信息。
...
调用 `create_payout` 提交出金请求。生成唯一 `request_id` 用于幂等控制，将 `payout_channel` 设置为 `PayoutChannel.OFFRAMP`，传入付款来源账户，并在 `recipient_info.bank_account_id` 中引用目标银行账户。
...
- [Upload file v2](/payments/en/api-references/payment/upload-file-v2)
- [Submit merchant KYC](/payments/en/api-references/payment/submit-merchant-kyc)
- [Create destination](/payments/en/api-references/payment/create-destination)
- [Create payout](/payments/en/api-references/payment/create-payout)
