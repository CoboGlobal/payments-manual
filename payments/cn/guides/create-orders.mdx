...
- 调用 [Create order link](/payments/en/api-references/payment/create-order-link) 创建一个订单支付链接。打开支付链接后，付款方将被引导至由 Cobo 托管的支付页面。付款方可以在该页面查看订单详情并完成支付，无需您自行开发前端交互流程。您还可以将支付页面通过 iFrame 方式嵌入到您的网站或应用中。
