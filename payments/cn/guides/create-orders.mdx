...
为优化您的账户成本，避免因粉尘金额产生的入账手续费高于交易金额，系统设定了最小入账阈值：**0.05 USDT**（或等值币种）。

**只有金额严格大于该阈值的交易才会入账。** 金额等于或低于阈值的交易按粉尘处理：不入账、不计入订单实收金额，也不会发送 Webhook 通知。
