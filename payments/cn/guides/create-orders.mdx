...
<Tabs>
  <Tab title="使用 Payments API 创建支付订单" icon="code">
    ...
    如果您是直接服务付款方的商户，通常只需设置 `psp_order_code`，无需填写 `merchant_order_code`。如果您是服务下游商户的平台或 PSP，请将 `psp_order_code` 设置为您自己的订单标识，并额外设置 `merchant_order_code` 为对应下游商户的订单编号，以便双方各自通过自己的标识对账。
    ...
    完整请求参数说明请参考 [Create pay-in order](/payments/en/api-references/payment/create-pay-in-order)。
    ...
    - **开发者费用**（`fee_amount`）：如果您是服务多个下游商户的平台机构，可以通过设置该订单级费用，从该笔订单中收取开发者分成。这与充值模式下商户级别的 `developer_fee_rate` 不同；关于该费率的配置说明，请参考[商户管理](/payments/cn/guides/merchants)。

      在正常结算情况下（即付款方的入账金额与应付金额完全一致），开发者账户将收到 `fee_amount`，商户账户将收到剩余部分：
      - 当 `fee_amount` 为 0 时，商户账户将收到全部收款金额。
      - 当 `payable_amount` 为 104.08、`fee_amount` 为 2 时，一笔精确的 104.08 入账将有 102.08 计入商户账户，2 计入开发者账户。

      对于多付和少付的情况，资金将按照 `fee_amount` 与应付金额之间的相同比例进行结算，而非始终扣减固定的 `fee_amount` 数值。如果某笔入账是在订单已经处于 `EXPIRED`、`UNDERPAID` 或 `COMPLETED` 状态之后才处理的，则该笔入账将全额计入开发者账户，不受 `fee_amount` 影响。

      更多关于账户间资金结算与分配的说明，请参考[账户与资金分配](/payments/cn/guides/amounts-and-balances)。
...
<Note>
  Create Refund API 适用于收款订单退款，不是收款方将已完成出金资金在链上退回后的恢复路径。关于收款订单退款、Payout reversal 和合规资金处置的区别，请参考[资金退回类型](/payments/cn/guides/create-refund-link#资金退回类型)。
</Note>
...
<Info>
  **退款流程**：`POST /payments/refunds`（Create Refund API）是开发者直接调用 API 发起的退款流程。您可以在创建请求中提供 `to_address`；若省略，退款订单将以 `PendingConfirmation` 状态创建，您需在处理前通过 Update refund order 操作提供地址。该 API 不会自动填入原始收款地址。这与[退款链接](/payments/cn/guides/create-refund-link)流程不同，后者由 Cobo 提供地址收集页面，并为付款方预填原始付款地址。Payout Reversal 及合规驱动的原路退回属于独立的运营和政策流程，不通过 `POST /payments/refunds` 发起。
</Info>
...
`Frozen` 是资金与合规处置状态，不是公开的 Payin 订单状态。公开的 Payin 订单状态包括 `Pending`、`Processing`、`Completed`、`Expired` 和 `Underpaid`。

如需跟踪 Payin 订单状态变更和交易事件，请参考[状态与事件](/payments/cn/guides/status-and-events)。如需观察冻结资金的处置状态，请调用 `get_disposition_status` 并检查 `disposition_status`。

#### 合规处置操作

以下操作是合规管理的资金处置操作，与开发者发起的 Payments 退款流程相互独立。

| 操作 | 用途 | 您可以观察的状态 |
| --- | --- | --- |
| `unfreeze_funds` | 合规审核后释放冻结资金。 | 调用 `get_disposition_status` 并检查 `disposition_status` 以跟踪结果状态。 |
| `isolate_funds` | 合规审核后隔离冻结资金。 | 调用 `get_disposition_status` 并检查 `disposition_status`；相关值为 `Isolating`、`Isolated` 和 `IsolateFailed`。 |
| `refund_funds` | 通过合规管理的返还流程退回冻结的 Payin 资金。 | 调用 `get_disposition_status` 并检查 `disposition_status` 以跟踪结果状态。此流程不同于开发者发起的 [Create Refund API 和退款链接流程](/payments/cn/guides/create-refund-link#退款场景)。 |

#### risk_level 可观察性

收款地址可能公开 `risk_level` 字段，该字段使用 `AddressRiskLevel` enum。您可以通过 [List crypto addresses](/payments/en/api-references/payment/list-crypto-addresses) 查询地址风险信息。风险信息可能是合规审核流程的一部分。

#### 责任边界

| 场景 | 开发者操作 |
| --- | --- |
| 查询 Payin 订单状态。 | 调用 [Get pay-in order information](/payments/en/api-references/payment/get-pay-in-order-information)，或订阅 `payment.order.status.updated`。 |
| 查询退款订单状态。 | 调用 [Get refund order information](/payments/en/api-references/payment/get-refund-order-information)、调用 [List all refund orders](/payments/en/api-references/payment/list-all-refund-orders)，或订阅 `payment.refund.status.updated`。 |
| 创建普通退款。 | 使用 Payments App，或调用 [Create refund order](/payments/en/api-references/payment/create-refund-order)。 |
| 审核 Screening App 案例。 | 使用 Screening App 中已验证的自助操作。 |
| 使用 `unfreeze_funds` 释放冻结资金。 | 这是合规管理的处置操作。调用 `get_disposition_status` 并检查 `disposition_status` 以观察结果状态。 |
| 使用 `isolate_funds` 隔离冻结资金。 | 这是合规管理的处置操作。调用 `get_disposition_status` 并检查 `disposition_status` 以观察结果状态。 |
| 使用 `refund_funds` 退回冻结的 Payin 资金。 | 这是合规管理的处置操作，与开发者发起的 [Create refund order](/payments/en/api-references/payment/create-refund-order) 相互独立。调用 `get_disposition_status` 并检查 `disposition_status` 以观察结果状态。 |

### 最小入账金额限制

为优化您的账户成本，避免因粉尘金额产生的入账手续费高于交易金额，系统设定了最小入账阈值。当单笔交易金额低于 **0.05 USDT**（或等值币种）时，系统将不自动进行入账处理。\
_注：若您需要对此类低于阈值的累积资金进行手动处理，请通过您的账户流程提交处理请求。_
