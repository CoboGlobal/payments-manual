...
<Note>
  Create Refund API 适用于收款订单退款，不适用于已完成的资金转出被退回后的处理。关于收款订单退款、资金转出被退回和合规资金处置的区别，请参考[资金退回类型](/payments/cn/guides/create-refund-link#资金退回类型)。
</Note>
...
<Info>
  **退款流程**：`POST /payments/refunds`（Create Refund API）是开发者直接调用 API 发起的退款流程。您可以在创建请求中提供 `to_address`；若省略，退款订单将以 `PendingConfirmation` 状态创建，您需在处理前通过 Update refund order 操作提供地址。该 API 不会自动填入原始收款地址。这与[退款链接](/payments/cn/guides/create-refund-link)流程不同，后者由 Cobo 提供地址收集页面，并为付款方预填原始付款地址。以下两个场景都不通过 `POST /payments/refunds` 发起，且彼此不同：**资金转出被退回**是您发起的资金转出完成后、该笔资金又被退回到 Payment 地址，资金流入，参考[加密货币转账](/payments/cn/guides/crypto-payouts#5-资金转出被退回)；**合规冻结资金退回**是入账资金被合规冻结后经合规资金处置流程退出，资金流出，使用 `refund_funds`。
</Info>
...
如需跟踪 Payin 订单状态变更和交易事件，请参考[状态与事件](/payments/cn/guides/status-and-events)。
...
| 操作 | 用途 | 说明 |
| --- | --- | --- |
| `unfreeze_funds` | 合规审核后释放冻结资金。 | 由合规团队管理。 |
| `isolate_funds` | 合规审核后隔离冻结资金。 | 由合规团队管理。 |
| `refund_funds` | 通过合规管理的返还流程退回冻结的 Payin 资金。 | 由合规团队管理。此流程不同于开发者发起的 [Create Refund API 和退款链接流程](/payments/cn/guides/create-refund-link#资金退回类型)。 |
...
| 使用 `unfreeze_funds` 释放冻结资金。 | 这是合规管理的处置操作，不属于普通的 Payments 退款流程。 |
| 使用 `isolate_funds` 隔离冻结资金。 | 这是合规管理的处置操作，不属于普通的 Payments 退款流程。 |
| 使用 `refund_funds` 退回冻结的 Payin 资金。 | 这是合规管理的处置操作，与开发者发起的 [Create refund order](/payments/en/api-references/payment/create-refund-order) 相互独立。 |

### 最小入账金额限制

为优化您的账户成本，避免因粉尘金额产生的入账手续费高于交易金额，系统设定了粉尘金额阈值。当单笔交易金额**低于或等于 0.05 USDT**（或等值币种）时，系统将不自动进行入账处理，该笔金额不计入订单实收金额，也不会发送 Webhook 通知。

举例：

| 单笔交易金额 | 是否入账 | 是否发送 Webhook |
|---|---|---|
| 0.06 USDT | 入账 | 发送 |
| **0.05 USDT** | **不入账** | **不发送** |
| 0.04 USDT | 不入账 | 不发送 |
