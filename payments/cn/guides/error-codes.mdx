...
|---|---|---|---|
| 1006 | 400 | 旧版共享 invalid-parameter 错误码，可能来自共享平台层或较早的集成。当前 Payments REST API 的 invalid-parameter 校验失败返回 `2006`。 | 将其视为 invalid-parameter 错误。检查 `error_message`，然后修正请求参数后重试。 |
...
| 2029 | 400 | 资源的当前状态不支持所请求的操作。常见原因：(1) 在交易达到 `Completed`、`Rejected` 或 `Failed` 状态之前尝试更新交易描述；(2) 尝试取消一个不处于 `Generating` 状态的交易导出任务；(3) 对于 [Create pay-in order](/payments/en/api-references/payment/create-pay-in-order)，您团队没有处于活跃状态的支付开发者账户——可能是账户记录不存在，也可能是账户记录存在但尚未激活，原因是尚未完成 [Payments 入驻](/payments/cn/guides/preparation)。 | 查看 `error_message` 字段了解具体原因。如需更新交易描述，请等待交易达到终态后再操作。如需取消导出任务，请确认该任务仍处于生成中状态。对于 Create pay-in order，这是团队级别的账户状态检查，与具体商户的状态或配置无关。请完成 [Payments 入驻](/payments/cn/guides/preparation)，使您团队的支付开发者账户变为活跃状态，然后重试请求。完整的前提条件列表请参考 [Create pay-in order 的前提条件](/payments/cn/guides/create-orders#前提条件)。 |
...
以下错误专属于 [Create Refund order](/payments/en/api-references/payment/create-refund-order) 操作（`POST /payments/refunds`）。所有错误均返回 HTTP 400 且 `"error_code": 2006`。使用响应中的 `error_message` 字段识别具体原因。

| 场景 | `error_message` 示例 | 原因 | 建议操作 |
|---|---|---|---|
| 重复的 `request_id` | 消息表明该 `request_id` 已被使用。 | 已存在使用此 `request_id` 的退款订单。 | 每次新建退款请求使用唯一的 `request_id`。如需查询已有退款，复用原 `request_id` 即可。 |
| `to_address` 无效 | `Invalid receiving address.` | `to_address` 格式与指定的 token 及链不兼容。 | 验证 `to_address` 是否符合该 token 所在网络的地址格式要求。 |
| 收款订单未找到 | `Order {order_id} not found.` | `order_id` 不存在或不属于指定的商户。 | 验证 `order_id` 是否正确，并确认该订单属于您指定的商户。 |
| Token 不匹配 | 消息表明退款 token 与收款订单 token 不一致。 | 请求中的 `token_id` 与原收款订单使用的 token 不同。 | 使用与原收款订单相同的 token。 |
| 商户未找到 | `Merchant {merchant_id} not found.` | `merchant_id` 在您的组织中不存在。 | 验证 `merchant_id` 是否正确，且属于您的组织。 |
| 商户不匹配 | 消息表明收款订单不属于指定的商户。 | `order_id` 存在，但属于不同的商户。 | 确认 `order_id` 和 `merchant_id` 指向同一商户。 |
| 余额不足 | 消息描述余额缺口。 | 指定的 `refund_type` 来源（商户余额或开发者余额）不足以覆盖退款金额及开发者费用。 | 提交前检查可用余额。根据情况调用 [List merchant balances](/payments/en/api-references/payment/list-merchant-balances) 或 [Get developer balance](/payments/en/api-references/payment/get-developer-balance)。 |
