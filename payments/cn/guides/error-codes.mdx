...
| 2027 | 400 | 您已超出此资源或操作的配额限制。 | 在 Cobo Portal 的**设置** > **开发者**下查看当前使用量和配额限制。 |
...
| 30031 | 400 | 该 token 未在您的组织中启用。 | 调用[获取支持的 token 列表](/payments/en/api-references/payment/list-supported-tokens)，验证该 token 是否对您的账户可用。如果列表中没有该 token，则表示该 token 尚未为您的组织激活。 |
