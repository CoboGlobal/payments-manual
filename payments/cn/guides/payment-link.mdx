...
- 订单支付链接的标准格式为：`https://payment-link.dev.cobo.com/payment?token=<token>`，其中 `<token>` 是 Cobo 为该托管支付页面生成的唯一标识符。此时尚未创建支付订单——只有当付款方在托管页面选择支付代币和区块链网络并提交后，Cobo 才会创建该订单。
...
sequenceDiagram
    ...
    订单支付链接 ->> Cobo: 提交订单
    Cobo ->> Cobo: 创建支付订单，生成 order_id
    Cobo -->> 您的应用后端: payment.order.status.updated Webhook（Pending，order_id）
...
当付款方在托管页面选择支付代币和区块链网络并提交订单后，Cobo 会创建该支付订单并生成 `order_id`。Cobo 会立即向您的后端发送一次 [`payment.order.status.updated`](/payments/cn/guides/status-and-events#paymentorderstatusupdated) Webhook 事件，状态为 `Pending`，并携带新生成的 `order_id`——这与后续随订单支付进度发送的 `payment.order.status.updated` 事件是不同的通知。完整的事件负载和触发条件请参考[事件与状态](/payments/cn/guides/status-and-events#paymentorderstatusupdated)。
